...
if (!is.na(slurm_ntasks)) {
  ...
  cores = parallel::detectCores() - 1 # Figure out how many cores there are
}
...
set.seed(1985 +  job_num)

j <- ncol(X)

res <- parallel::mclapply(
  max(1, 20 * (job_num - 1) + 1):min(j, 20 * job_num),
  function(i){
    X_tmp <- X
...
# tictoc::toc()
write_rds(res, glue::glue("results/abide_vip/abide_vip_sim_{job_num}.Rds"))
