  ...
  if(filter){
    if(plot_type == "rsn" & !is.null(vip)){
      ...
      data_sig <- left_join(data, vip_dat, by = c("source", "target", "pred")) %>%
        mutate(
          value1 = ifelse(value==0, NA, value),
          value = ifelse(abs(value1) > quantile(abs(value1), 0.75, na.rm = TRUE), value, NA)
          # value = ifelse(vip > quantile(filter_val, 0.75, na.rm = TRUE), value, NA)
          ) %>%
        drop_na()
...
create_regression_coeff_plots <- function(B, L, dataset = "cobre",
  ...
  if(!is.null(vip_vals)){
    ...
    vip_mat <- vip_mat %>%
      as_tibble() %>% 
      # as_tibble(rownames = "pred") %>%
      mutate(pred=sig$pred,
             R = str_split(pred, pattern = "-"),
  ...
  p <- create_corrplot(P, title, filter = filter, plot_type = "rsn", height = height, vip = vip_sub, dataset=dataset)
  # p <- corrplot(P, is.corr = FALSE, type = "lower",
  #               col = COL2("RdBu", 200), title = title,
  #               mar=c(0,0,2,0))
